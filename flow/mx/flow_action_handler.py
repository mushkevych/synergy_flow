...
def entry_schedulable_name(flow_name, step_name):
    """ forms a freerun workflow runtime key for `FreerunProcessEntry.entry_name` """
    return '{0}::{1}'.format(flow_name, step_name)


def full_schedulable_name(process_name, flow_name, step_name):
    """ used as a substitution for freerun `uow.process_name`
        replicates format of the `FreerunProcessEntry.schedulable_name` with an assumption
        that entry_name is generated by `entry_schedulable_name` """
    entry_name = entry_schedulable_name(flow_name, step_name)
    return '{0}::{1}'.format(process_name, entry_name)
...
def find_flow_step_uow(uow_dao, process_name, flow_name, step_name, timeperiod):
    ...
    try:
        schedulable_name = full_schedulable_name(process_name, flow_name, step_name)
...
def find_all_flow_uows(uow_dao, process_name, flow_name, timeperiod):
    assert isinstance(uow_dao, UnitOfWorkDao)
    schedulable_stem = full_schedulable_name(process_name, flow_name, '')
    query = QUERY_GET_FLOW_FREERUNS(schedulable_stem, timeperiod)
    try:
        uows = uow_dao.run_query(query)
    except:
        uows = list()
    return uows
...
class FlowRequest(object):
    ...
    def schedulable_name(self):
        return full_schedulable_name(self.process_name, self.flow_name, self.step_name)
...
class FlowActionHandler(BaseRequestHandler):
    ...
    def freerun_uow_records(self):
        return find_all_flow_uows(self.uow_dao, self.process_name, self.flow_name, self.timeperiod)
